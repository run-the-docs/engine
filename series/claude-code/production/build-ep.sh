...
cp "$DEMO/rec/narration.json" "$DEMO/rec/narration-ep${N}.json"; cp "$DEMO/rec/narration.wav" "$DEMO/rec/narration-ep${N}.wav"  # cache for render-only reruns
...
ls -la "$DEMO/rec/claude-ep${N}-45.mp4" "$DEMO/rec/claude-ep${N}-916.mp4"
echo "EP${N}BUILD_OK"

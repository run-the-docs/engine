...
case "$EP" in
 ...
 25) RESET=5411d3b; LAUNCH_FIRST=1; ARTIFACTS=""; CLAUDE='claude --strict-mcp-config --allowedTools Read --disallowedTools WebFetch WebSearch'; MODE=slash; TEXT="/fast on"; COMPLETE=stable ;;
 ...
 38) RESET=5411d3b; ARTIFACTS=""; MODE=bash; CLAUDE=':'; TEXT='claude --strict-mcp-config --model haiku --allowedTools Read --disallowedTools WebFetch WebSearch -p "In one sentence, what does slugify.py do?"'; COMPLETE=stable ;;
esac

...
if not (art and os.path.exists(apath)) and cmd:
    ...
    inner_w=TWID-40; cw=max(1,tw(ad,"M",cf)); maxc=max(8,inner_w//cw)   # chars that fit the card width
    ybot=TY+THGT-6; maxlines=max(1,(ybot-ystart)//lh)
    def _hw(s):                                     # wrap to card width: break on spaces, preserve indent, hard-cut only long tokens
        if len(s)<=maxc: return [s]
        ind=len(s)-len(s.lstrip(" ")); pad=" "*min(ind+2,maxc-4)
        out=[]; cur=s
        while len(cur)>maxc:
            cut=cur.rfind(" ",len(pad),maxc)        # last space within the width (after the indent)
            if cut<=len(pad): cut=maxc              # no usable space -> hard cut

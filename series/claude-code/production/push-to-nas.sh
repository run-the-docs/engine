#!/bin/bash
# Archive produced Run-the-Docs videos to the Synology DS412+ (Tailscale <nas-tailscale-ip>, ~9TB).
#
# Prereq: a shared folder on the NAS mounted on this Mac. One-time mount, e.g.:
#   SMB:  mount_smbfs //USER@<nas-tailscale-ip>/video /Volumes/video
#   NFS:  sudo mount -t nfs -o resvport,nolocks <nas-tailscale-ip>:/volume1/video /Volumes/video
...
if [ ! -d "$NAS_MOUNT" ]; then
  ...
  echo "  mount_smbfs //USER@<nas-tailscale-ip>/video $NAS_MOUNT"
  echo "  (or NFS: sudo mount -t nfs -o resvport,nolocks <nas-tailscale-ip>:/volume1/video $NAS_MOUNT)"
  exit 1
fi

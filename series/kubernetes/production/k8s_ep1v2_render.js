#!/usr/bin/env node
/**
 * Render k8s_ep1v2.html using Playwright + canvas.toDataURL()
...
const OUT_DIR = '/tmp/k8s_ep1v2_frames';
...
async function main() {
  ...
  const htmlPath = `file:///tmp/k8s_ep1v2.html`;
  ...
  // Wait for logo image to load (onload fires)
  ...
  for (const check of checks) {
    const result = await page.evaluate((frameNum) => {
      ...
      return { 
        ...
        logoLoaded: window.logoLoaded
      };
    }, check.frame);
    console.log(`  ${check.label}: rgb(${result.r},${result.g},${result.b}) logo=${result.logoLoaded} ${result.isBlack ? '⚠️ BLACK' : '✓ OK'}`);
  }

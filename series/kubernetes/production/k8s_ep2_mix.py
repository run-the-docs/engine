#!/usr/bin/env python3
"""Mix TTS voice + SFX + background music for K8s Ep2"""
import numpy as np
...
# Load timing
with open('/tmp/k8s_ep2_timing.json') as f:
    timing_data = json.load(f)
total_dur = timing_data['total_duration']

# Load voice
voice = load_wav('/tmp/k8s_ep2_voice.wav', SR)
...
# Sound design
print("Mixing SFX...")
mix_sfx('/tmp/k8s_ep2_impact_thud.wav',     insert_time_s=2.0,  volume=0.55)   # hook reveal
mix_sfx('/tmp/k8s_ep2_ascending_chime.wav', insert_time_s=76.5, volume=0.45)   # payoff
...
output_path = '/tmp/k8s_ep2_mixed.wav'

#!/usr/bin/env python3
"""Generate TTS for Kubernetes Ep10: StatefulSets"""
...
sentences = [
    # HOOK (0-5s)
    "MySQL, Kafka, Redis. They can't just be any Pod — they need to be the same Pod. That's StatefulSets.",
    # DEPLOYMENT vs STATEFULSET (5-20s)
    "Deployment Pods are interchangeable. Random names. Replaceable.",
    "StatefulSet Pods are sticky. mydb-0 is always mydb-0.",
    "Same name, same storage, same identity — even across rescheduling.",
    # ORDERED OPERATIONS (20-35s)
    "Pods are created in order: zero, then one, then two.",
    "Scale down in reverse. Rolling updates one at a time.",
    "This prevents split-brain in clustered databases.",
    # PERSISTENT STORAGE (35-48s)
    "Each Pod gets its own PersistentVolumeClaim.",
    "Deleting the StatefulSet does not delete the volumes. Your data stays safe.",
    # PAYOFF (48-62s)
    "Stateless workload? Use a Deployment.",
    "Needs stable identity and persistent storage? StatefulSet.",
    "Databases live here.",
    # OUTRO (62-72s)
    "Run the Docs. Kubernetes. StatefulSets.",
]
...
output_path = '/tmp/k8s_ep10_voice.wav'
...
timing_path = '/tmp/k8s_ep10_timing.json'

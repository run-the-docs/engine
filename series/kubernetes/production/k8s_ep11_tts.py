#!/usr/bin/env python3
"""Generate TTS for Kubernetes Ep11: DaemonSets & Jobs"""
...
sentences = [
    # HOOK (0-5s)
    "Two controllers. One runs on every node. One runs once and stops.",
    # DAEMONSET (5-28s)
    "The DaemonSet ensures every node in your cluster runs a copy of a Pod.",
    "Add a node — a Pod appears. Remove a node — it's gone.",
    "Log shippers. Monitoring agents. Network plugins. kube-proxy.",
    "Infrastructure that has to live everywhere.",
    "That's a DaemonSet.",
    # JOBS (28-52s)
    "A Job creates Pods and runs until a specified number complete successfully.",
    "Not a service. A task with a finish line.",
    "Batch processing. Data migration. Machine learning training runs.",
    "Retries on failure. Delete the Job — Pods are cleaned up.",
    "Need it on a schedule? That's a CronJob.",
    # PAYOFF (52-62s)
    "DaemonSet: runs everywhere. Job: runs once.",
    "Different tools for different shapes of work.",
    # OUTRO (62-75s)
    "Run the Docs. Kubernetes. DaemonSets and Jobs.",
]
...
output_path = '/tmp/k8s_ep11_voice.wav'
...
timing_path = '/tmp/k8s_ep11_timing.json'

#!/usr/bin/env node
/**
 * Render k8s_ep2v2.html using Playwright + canvas.toDataURL()
...
async function main() {
  ...
  const htmlPath = `file:///tmp/k8s_ep2v2.html`;
  ...
  const checks = [
    ...
    { frame: 90,   label: 't=3 → hook scene (CP + nodes)' },
    { frame: 360,  label: 't=12 → control plane cards' },
    { frame: 1680, label: 't=56 → node components' },
    { frame: 2310, label: 't=77 → payoff scene' },
  ];
